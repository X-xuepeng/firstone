...
Page({
          ...
          if (res.data.Code === 200) {
            if (res.data.Datas.length !== 0) {
              for (let data of res.data.Datas){
                data.checked=false;
                data.showCount = data.AllowRetrunNumbers;
              }
              that.setData({
  ...
  onConfirm: function () {
    ...
    if (this.data.materialCount > clickItem[0].AllowRetrunNumbers){
      wx.showToast({
        title: '已超过允许退的最大值' + clickItem[0].AllowRetrunNumbers,
        icon: 'none'
      })
      return
    }
  ...
  onClickButton:function(e){
    ...
    for (let material of this.data.checkData){
     ...
     wx.showLoading({
      ...
      wx.request({
        url: 'https://api.gopas.com.cn/api/WorkShop/ReturnStroreApply',
        method:'post',
        data:{
